...
public class BloomFilter<T> implements Serializable {
    ...
    public void addViaMurmur(T element) {
        ...
        Arrays.stream(hashes).forEach(hash -> bitset.set(Math.abs((int) (hash % bitSetSize)), true));
        numberOfAddedElements++;
    }

    public void add(byte[] bytes) {
        int[] hashes = hash(bytes, numberOfHashFunctions);
        Arrays.stream(hashes).forEach(hash -> bitset.set(Math.abs(hash % bitSetSize), true));
        numberOfAddedElements++;
    }
    ...
    public boolean contains(byte[] bytes) {
        ...
        return Arrays.stream(hashes)
                .allMatch(hash -> bitset.get(Math.abs(hash % bitSetSize)));
    }
    ...
    public boolean containsViaMurmur(byte[] bytes) {
        ...
        return Arrays.stream(hashes)
                .allMatch(hash -> bitset.get(Math.abs((int) (hash % bitSetSize))));
    }

    public double getFalsePositiveProbability(double numberOfElements) {
        // (1 - e^(-k * n / m)) ^ k
        //  p = pow(1 - exp(-k / (m / n)), k)
        return Math.pow((1 - Math.exp(-numberOfHashFunctions * numberOfElements / bitSetSize)), numberOfHashFunctions);
    }
