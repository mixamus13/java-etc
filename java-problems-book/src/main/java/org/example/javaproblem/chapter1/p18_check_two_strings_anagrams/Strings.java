...
class AppSberInterviewTree {
    public static void main(String... args) {
        ...
        var hashTags = List.of(
                ...
                "В Молдове городе #Рыбница есть винограда тона"
        );
...
На вход метода передаётся коллекция сообщений, которые состоят из текста и хэштэгов.
Мы точно знаем, что все слова и хэштэги разделены пробелами.
На выходе нужно отдать отсортированную по алфавиту коллекцию уникальных хэштэгов, которые использовались в этих сообщениях.
Уникальность должна быть регистронезависимой, т.е. все хэштэги привести к одному регистру
(например, считать, что #МОСКВА и #москва - одно и то же).

Сегодня в #МОСКВА хорошая погода, а в #Питер опять дождь
В #питер сегодня пробки 10 баллов
В Молдове городе #Рыбница есть много винограда
 */
